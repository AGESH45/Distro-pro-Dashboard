...
export function Register({ onLogin }: { onLogin: () => void }) {
  ...
  const validate = () => {
    ...
    if (password.length < 6) nextErrors.password = mode === 'register' ? 'Password is required.' : 'Password must be at least 6 characters.';
    if (mode === 'register') {
      ...
      <section className="relative mx-auto grid min-h-screen max-w-7xl items-center gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[1fr_.88fr] lg:px-8">
        ...
        <form onSubmit={submit} className="mx-auto w-full max-w-xl rounded-[2rem] border border-white/10 bg-white/[0.08] p-5 shadow-2xl shadow-black/30 backdrop-blur md:p-8">
          ...
          <div className="mt-6 space-y-4">
            ...
            <button disabled={submitting} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-red-600 px-5 py-4 font-black text-white shadow-lg shadow-blue-950/40 disabled:opacity-60">{submitting && <Loader2 className="h-5 w-5 animate-spin" />} {mode === 'login' ? 'Sign in securely' : 'Create account'}</button>
            ...
            <button type="button" onClick={googleSignIn} disabled={googleLoading} className="flex w-full items-center justify-center gap-3 rounded-2xl border border-white/15 bg-white/[0.04] px-5 py-4 font-black text-white hover:bg-white/10 disabled:opacity-60">
              {googleLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <span className="grid h-6 w-6 place-items-center rounded-full bg-white text-sm font-black text-slate-950">G</span>}
              Continue with Google
            </button>
            <p className="text-center text-xs leading-5 text-white/45">No demo access. Production authentication is handled by Supabase Auth.</p>
          </div>
        </form>
      </section>
    </main>
