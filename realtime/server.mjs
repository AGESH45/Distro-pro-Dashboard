...
const PORT = Number(process.env.PORT || process.env.REALTIME_PORT || 8080);
...
function subscribe(client, topic) {
  if (topic !== 'admin' && !topic.startsWith('artist:')) return;
...
const server = http.createServer((req, res) => {
  if (req.method === 'POST' && req.url === '/publish') {
    ...
    req.on('end', () => {
      try {
        ...
        if (!body.topic || !body.type) throw new Error('topic and type are required');
  ...
  res.writeHead(200, { 'content-type': 'application/json' }); res.end(JSON.stringify({ ok: true, service: 'trm-realtime' }));
});
...
wss.on('connection', (ws, req) => {
  ...
  if (!user?.id) return ws.close(1008, 'Invalid realtime token');
  const client = { ws, user, topics: new Set() };
  ws.send(JSON.stringify({ type: 'realtime.ready', topic: 'system', timestamp: new Date().toISOString() }));
  ws.on('message', raw => {
    try {
      const message = JSON.parse(String(raw));
      for (const topic of message.topics || []) {
        if (message.action === 'subscribe') subscribe(client, topic);
        if (message.action === 'unsubscribe') unsubscribe(client, topic);
      }
    } catch { /* ignore malformed client messages */ }
  });
  ws.on('close', () => { for (const topic of client.topics) unsubscribe(client, topic); });
});
